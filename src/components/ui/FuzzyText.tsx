...
export function FuzzyText({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Resizing the canvas resets its 2D context, so the font must be re-set.
      ...
      // init() is async: the effect may already have been torn down by the time
      // the font finished loading, in which case nothing above should stay live.
      if (isCancelled) {
